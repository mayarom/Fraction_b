...
TEST_CASE("Fraction with largest possible numerator and/or denominator and overflow handling") {
    ...
    CHECK_NOTHROW(Fraction f1(MAXINT, 1));
    Fraction f1(MAXINT, 1);
    CHECK_EQ(f1, Fraction(MAXINT, 1));

    // Test largest possible denominator
    CHECK_NOTHROW(Fraction f2(1, MAXINT));
    Fraction f2(1, MAXINT);
    CHECK_EQ(f2, Fraction(1, MAXINT));

    // Test largest possible numerator and denominator
    CHECK_NOTHROW(Fraction f3(MAXINT, MAXINT));
    Fraction f3(MAXINT, MAXINT);
    CHECK_EQ(f3, Fraction(1, 1));

    // Test arithmetic with large numerator and/or denominator
    Fraction f4(MAXINT - 100, MAXINT);
    ...
    Fraction f5(MAXINT - 1, 1);
    Fraction f6(MININT, 1);
    Fraction f7(MININT + 1, 1);
